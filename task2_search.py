...
# You can switch to your project's utility if you want:
# from src.retrieval_graph.utils import load_chat_model
# Here we keep it standalone (only openai) to reduce extra deps.
try:
...
def main():
  ...
  # 确保根目录下有 outputs 文件夹
    ...
    # 保存结果到 outputs/task2_llm_results.json
    write_json(os.path.join("outputs", "task2_llm_results.json"), {
        "query": args.query,
        "model": args.model,
        "schemas": args.schemas,
        "choices": ranked
    })
    print("Saved: outputs/task2_llm_results.json")
